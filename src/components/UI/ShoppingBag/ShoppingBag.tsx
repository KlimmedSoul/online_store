...
import cl from './ShoppingBag.module.css';
...
const ShoppingBag: React.FC = () => { 
    ...
    if(allProducts !== undefined) {
        for(let i = 0; i < allProducts.length; i++) {
            total += allProducts[i].count * allProducts[i].price
        }
    }

    
    const countHandler = (flag: boolean, name: string, size: string, color: string) => {
        const updatedProducts = allProducts.map((prod:Product) => {
            if(prod.count === 1 && !flag) {
                return prod
            }
            if(prod.name === name && prod.size === size && prod.color === color) {
                if(flag) {
                   return {
                    ...prod,
                    count: prod.count + 1
    ...
    const deleteHandler = (name: string, size: string, color: string, pic:string, price:number, count:number) => {
        let updatedProducts = allProducts
        let index = updatedProducts.findIndex((product:Product) => 
            product.name === name &&
            product.size === size &&
            product.color === color &&
            product.pic === pic &&
    ...
    const orderHandler = () => {
        if(checked) {
            ...
            alert("Согласитесь с условиями торговой площадки")
        }
    }
    ...
    return (
        <div className={cl.root}>
            <div className={cl.header}> 
                ...
                <p className={cl.all_count}>{ allProducts === null ? '0' : allProducts.length}</p>
            </div>

            <div className={cl.products_handler}>
                {allProducts !== null ? allProducts.map((prod: Product, index:number) => (
                    <div className={cl.prod_card} >

                        <div className={cl.ico_handler}>
                            <img src={prod.pic} alt="123" className={cl.prod_ico}/>
                        </div>

                        <div className={cl.info_handler} >

                            <div className={cl.info_title_handler}>
                                <h3 className={cl.prod_name}>{prod.name}</h3>
                                <p className={cl.art}>Арт 09456784</p>
                            </div>

                            <div className={cl.color_size_handler}>
                                <p className={cl.color_size_text}>Цвет: {prod.color}</p>
                                <p className={cl.color_size_text}>Размер: {prod.size}</p>
                            </div>

                            <div className={cl.plus_and_minus}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" className={cl.minus}
                                onClick={() => countHandler(false, prod.name, prod.size, prod.color)}>
                                ...
                                <h3 className={cl.prod_count}>{prod.count}</h3>

                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" className={cl.plus}
                                onClick={() => countHandler(true, prod.name, prod.size, prod.color)}>
                            ...
                            <div className={cl.favorite_and_remove}>
                                <div className={cl.far_handler}>
                                    ...
                                    <label className={cl.title_btns}>В избранное</label>
                                </div>
                                
                                <div className={cl.far_handler} onClick={() => deleteHandler(prod.name, prod.size, prod.color, prod.pic, prod.price, prod.count)}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" >
                                    ...
                                    <label className={cl.title_btns}>Удалить</label>
                                </div>
                            </div>

                        </div>

                        <div className={cl.cost_handler}>
                            <h1 className={cl.cost_title}>Стоимость</h1>
                            <h1 className={cl.cost}>{prod.price} Р</h1>
                            <div className={cl.delivery_info}>
                                <h3 className={cl.delivery_info_title}>Информация о доставке</h3>
                                <svg xmlns="http://www.w3.org/2000/svg" width="12" height="8" viewBox="0 0 12 8" fill="none" className={cl.arrow_info}>
            ...
            <div className={cl.order_handler}>
                <div className={cl.total_handler}>
                    <h1 className={cl.total_title}>Итого: <span className={cl.total}>{total} Р</span></h1>
                    <p className={cl.all_prod_count}>Товары, {allProducts === null ? '0' : allProducts.length} шт.</p>
                </div>

                <button className={cl.button} 
                onClick={orderHandler}>
                    <label className={cl.button_title}>Заказать</label>
                </button>

                <div className={cl.checkbox_handler}>
                    <input type="checkbox" className={cl.checkbox} onChange={checked === false ? () => setChecked(true) : () => setChecked(false)}/>
                    <label className={cl.checkbox_text}>Согласен с условиями правил пользования торговой площадкой и правилами возврата</label>
                </div>
